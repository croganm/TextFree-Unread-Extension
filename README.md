...
## Self-hosted CRX (Google Workspace / Admin)

This repo can publish a signed **`.crx`** and **`updates.xml`** on `main` via GitHub Actions ([`.github/workflows/release-extension.yml`](.github/workflows/release-extension.yml)) for installation through **Google Admin** (extension ID + update URL).

| What | Value |
|------|--------|
| **Extension ID** | `kokcmkomlkjlolnoklgikcmkehdjedim` (from the signing key; must match Admin) |
| **Update URL** | `https://raw.githubusercontent.com/croganm/TextFree-Unread-Extension/main/updates.xml` |
| **Artifacts** | [`TextFree-Mark-Unread.crx`](TextFree-Mark-Unread.crx), [`updates.xml`](updates.xml) on `main` |

**Repository secret (required for Actions):** add **`CRX_PRIVATE_KEY_B64`** (recommended) or **`CRX_PRIVATE_KEY`** — the PEM used to sign the CRX, same key as for local `npm run pack`. Never commit the `.pem` (it is gitignored).

**Branch protection:** if `main` is protected, allow the `github-actions[bot]` user to push, or the workflow cannot commit new artifacts.

**Note:** Dragging the `.crx` onto `chrome://extensions` may fail with `CRX_REQUIRED_PROOF_MISSING`; that is expected for developer-signed packages. Use **Load unpacked** for local dev, or deploy via Admin.

## Project layout

| Path | Purpose |
|------|---------|
| `TextFree-Mark-Unread.crx`, `updates.xml` | Signed package + update manifest for Admin (regenerated by CI) |
